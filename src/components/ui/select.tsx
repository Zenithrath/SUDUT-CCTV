...
> & {
  ...
  return (
    <PopoverPrimitive
      ...
      offset={offset}
      crossOffset={crossOffset}
      className={cn("isolate z-50 w-(--trigger-width) min-w-36 rounded-lg bg-popover text-popover-foreground shadow-lg ring-1 ring-border duration-100 data-entering:animate-in data-entering:fade-in-0 data-entering:zoom-in-95 data-exiting:animate-out data-exiting:fade-out-0 data-exiting:zoom-out-95 data-[placement=bottom]:slide-in-from-top-2 data-[placement=left]:slide-in-from-right-2 data-[placement=right]:slide-in-from-left-2 data-[placement=top]:slide-in-from-bottom-2 animate-none! **:data-[slot$=-item]:data-focused:bg-muted **:data-[slot$=-item]:focus:bg-muted **:data-[slot$=-item]:data-highlighted:bg-muted **:data-[slot$=-separator]:bg-border **:data-[slot$=-trigger]:focus:bg-muted **:data-[slot$=-trigger]:aria-expanded:bg-muted **:data-[variant=destructive]:focus:bg-destructive/10 **:data-[variant=destructive]:text-accent-foreground **:data-[variant=destructive]:**:text-accent-foreground", className )}
      {...props}
    >
      {children}
    </PopoverPrimitive>
...
function SelectItem({
  ...
  return (
    <ListBoxItemPrimitive
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2.5 rounded-md py-2 pr-8 pl-3 text-sm font-medium outline-hidden select-none focus:bg-muted focus:text-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground data-focused:bg-muted data-focused:text-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className
      )}
      {...props}
    >
